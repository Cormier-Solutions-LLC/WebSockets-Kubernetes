...
`Update-RealtimeVersion.ps1 -Version <semantic-version>` updates the current coordinated version across active tracked files, prepends entries to each reference-application changelog, and creates the matching release-notes stub. It deliberately preserves historical release notes, existing changelog entries, and `refs`. Run it with `-WhatIf` first, then edit the generated summaries and validate the resulting artifacts before committing.
