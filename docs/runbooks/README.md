...
2. Configure the repository variables `CONTAINER_REGISTRY`, `IMAGE_REPOSITORY`, and `REGISTRY_CREDENTIAL_PROVIDER`. Set the provider to `harbor`, `dockerhub`, or `generic` and configure its corresponding protected secret pair: `HARBOR_USERNAME`/`HARBOR_TOKEN`, `DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN`, or `REGISTRY_USERNAME`/`REGISTRY_PASSWORD`. Publishing fails closed when the provider is unsupported or any registry coordinate or selected credential is absent. Retrieve `release-manifest.json` and verify its source commit, archive SHA-256, repository, and registry digest.
