...
`bootstrap` restores and builds the project-reference development graph, then packs the libraries into an isolated local feed and builds the same application using only NuGet package references. A missing browser static-asset route fails bootstrap. `validate` runs bounded Playwright scenarios. `cleanup` deletes only the configured test prefix in Redis and the repository's `artifacts/full-circle` directory. Repeating bootstrap, validation, or cleanup is supported.
