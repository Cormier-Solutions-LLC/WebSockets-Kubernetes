...
Artisan::command('app:preflight', function (ReferenceConfig $config): int {
    try {
        ...
        foreach ([$config->sharedAssetRoot.'/index.html', $config->sharedAssetRoot.'/app.css', $config->sharedAssetRoot.'/app.js', $config->sdkAssetRoot.'/cormier-realtime.iife.js', $config->sdkAssetRoot.'/cormier-realtime.iife.min.js'] as $path) {
            if (! is_file($path)) {
