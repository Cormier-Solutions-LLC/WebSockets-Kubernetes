...
Install from your approved NuGet source with `dotnet add package Cormier.Realtime.Contracts --version 0.1.0`. The .NET Standard target depends on System.Text.Json `[10.0.12,11.0.0)`; the .NET 10 target uses its framework implementation. No ASP.NET Core or Redis runtime is required by this package.
