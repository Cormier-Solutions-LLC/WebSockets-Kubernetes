...
Prometheus 0.0.4 and OpenMetrics 1.0 are negotiated on the configured `Metrics__Path` (default `/metrics`). Gzip is supported through response compression. Restrict the scrape path with `Metrics__AuthorizationPolicy`, `Metrics__AllowedNetworks`, TLS at the edge, and NetworkPolicy as required. The standalone bearer option reads `Metrics__ScrapeToken`; the Helm chart supplies it from `metrics.scrapeTokenSecret`, which must be distinct from the diagnostics operator credential. Enable OTLP with `Metrics__OtlpEnabled=true`; endpoint, protocol, headers, compression, timeout, TLS, and resource settings use standard `OTEL_*` variables. Secret OTLP headers must come from a Secret, never a ConfigMap or values file.
