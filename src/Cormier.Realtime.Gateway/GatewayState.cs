...
public sealed class GatewayMetrics : IDisposable
{
    ...
    public string RenderPrometheus() =>
        "# HELP cormier_realtime_health_requests_total Health endpoint requests.\n" +
        "# TYPE cormier_realtime_health_requests_total counter\n" +
        $"cormier_realtime_health_requests_total {HealthRequestCount}\n" +
        "# HELP cormier_realtime_active_connections Current WebSocket connections.\n" +
        "# TYPE cormier_realtime_active_connections gauge\n" +
        $"cormier_realtime_active_connections {ActiveConnections}\n" +
        "# TYPE cormier_realtime_messages_total counter\n" +
        $"cormier_realtime_messages_total {Interlocked.Read(ref _messageCount)}\n" +
        "# TYPE cormier_realtime_authentication_failures_total counter\n" +
        $"cormier_realtime_authentication_failures_total {Interlocked.Read(ref _authenticationFailures)}\n" +
        "# TYPE cormier_realtime_authorization_failures_total counter\n" +
        $"cormier_realtime_authorization_failures_total {Interlocked.Read(ref _authorizationFailureCount)}\n" +
        "# TYPE cormier_realtime_queue_dropped_total counter\n" +
        $"cormier_realtime_queue_dropped_total {Interlocked.Read(ref _queueDropCount)}\n" +
        "# TYPE cormier_realtime_redis_errors_total counter\n" +
        $"cormier_realtime_redis_errors_total {Interlocked.Read(ref _redisErrorCount)}\n" +
        "# TYPE cormier_realtime_websocket_closes_total counter\n" +
        $"cormier_realtime_websocket_closes_total {Interlocked.Read(ref _closeCount)}\n" +
        "# TYPE cormier_realtime_handler_cancellations_total counter\n" +
        $"cormier_realtime_handler_cancellations_total {Interlocked.Read(ref _handlerCancellationCount)}\n" +
        "# TYPE cormier_realtime_draining gauge\n" +
        $"cormier_realtime_draining {Interlocked.Read(ref _draining)}\n";
