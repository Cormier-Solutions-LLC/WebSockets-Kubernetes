...
Pass `-NameSuffix <dns-label>` to derive a consistent instance identity such as `realtime-customer-a`; the ignored `.bootstrap/naming.json` manifest contains the corresponding service, container, Redis-prefix, and Kubernetes application names.
