...
The package targets `net10.0`, depends on StackExchange.Redis `3.1.31`, and carries a compatible Contracts dependency (default range `[1.0.0,1.1.0)`). Install from your configured NuGet source with `dotnet add package Cormier.Realtime.Redis --version 1.0.2-beta`. It does not provision Redis, configure ACLs, register gateway endpoints or supply a login flow.
