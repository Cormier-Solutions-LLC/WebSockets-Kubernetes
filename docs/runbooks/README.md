...
2. Configure the repository variables `CONTAINER_REGISTRY` and `IMAGE_REPOSITORY` and the protected secrets `HARBOR_USERNAME` and `HARBOR_TOKEN`; publishing fails closed when any registry coordinate or credential is absent. Retrieve `release-manifest.json` and verify its source commit, archive SHA-256, repository, and registry digest.
